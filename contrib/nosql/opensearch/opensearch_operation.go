...
const (
	...
	// openSearchDistribution is the distribution marker returned by OpenSearch Info.
	openSearchDistribution = "opensearch"
)

// Ping verifies whether the OpenSearch cluster is reachable.
func (o *OpenSearch) Ping(ctx context.Context) error {
	response, err := o.performHTTP(ctx, gsearch.RequestMethodHead, "/", nil, nil, nil, "")
	if err != nil {
		return err
	}
...
// Info returns basic OpenSearch cluster information.
func (o *OpenSearch) Info(ctx context.Context) (*gsearch.InfoResponse, error) {
	response, err := o.performHTTP(ctx, gsearch.RequestMethodGet, "/", nil, nil, nil, "")
	if err != nil {
		return nil, err
	}
	...
	if err = validateOpenSearchProduct(payload); err != nil {
		return nil, err
	}
...
// Perform executes a raw OpenSearch request.
func (o *OpenSearch) Perform(ctx context.Context, req *gsearch.Request) (*gsearch.Response, error) {
	if req == nil {
		req = &gsearch.Request{}
	}
	method := req.Method
	...
	return o.performHTTP(ctx, method, req.Path, req.Query, req.Headers, req.Body, contentTypeJSON)
}

// Search executes an OpenSearch search request.
func (o *OpenSearch) Search(ctx context.Context, req *gsearch.SearchRequest) (*gsearch.SearchResponse, error) {
	if req == nil {
		req = &gsearch.SearchRequest{}
	}
	response, err := o.performHTTP(
		ctx,
...
// Bulk executes an OpenSearch bulk request.
func (o *OpenSearch) Bulk(ctx context.Context, req *gsearch.BulkRequest) (*gsearch.BulkResponse, error) {
	if req == nil {
...
// Close releases OpenSearch adapter resources.
func (o *OpenSearch) Close(_ context.Context) error {
	_, err := o.ensureClient()
	return err
}

// ensureClient returns the initialized official client.
func (o *OpenSearch) ensureClient() (*opensearchv3.Client, error) {
	if o == nil {
		return nil, gerror.NewCode(gcode.CodeInvalidConfiguration, "opensearch adapter is nil")
	}
	if o.initErr != nil {
		return nil, o.initErr
	}
	if o.client == nil {
		return nil, gerror.NewCode(gcode.CodeInvalidConfiguration, "opensearch client is nil")
	}
	return o.client, nil
}

// performHTTP executes an HTTP-style request through the official client transport.
func (o *OpenSearch) performHTTP(
	ctx context.Context,
...
func responseFromHTTP(response *http.Response) (*gsearch.Response, error) {
	if response == nil {
		return nil, gerror.NewCode(gcode.CodeInvalidConfiguration, "opensearch response is nil")
	}
	var body []byte
...
// validateOpenSearchProduct rejects non-OpenSearch Info responses.
func validateOpenSearchProduct(payload map[string]any) error {
	version := gconv.Map(payload["version"])
	distribution := gconv.String(version["distribution"])
	if distribution == openSearchDistribution {
		return nil
	}
	return gerror.NewCodef(
		gcode.CodeInvalidConfiguration,
		"unsupported OpenSearch distribution %q",
		distribution,
	)
}
...
func errorForStatus(response *gsearch.Response) error {
	if response != nil && response.StatusCode > 299 {
		return gerror.NewCodef(
			...
			"opensearch response status %d: %s",
			response.StatusCode,
